...
def update_table():
    ...
    table.update_values(table_data, list_range = f'result!A{table_i_start}:P{len(table_data)}')
...
def process_table():
    global table_data
    if table_i_start == 1: table_data.append(('url', 'query', 'oneline_sitelinks', 'oneline_sitelinks', 'oneline_sitelinks', 'oneline_sitelinks', 'oneline_sitelinks', 'oneline_sitelinks', 'oneline_sitelinks'))
    i_row = 0
    for filename in os.listdir('data'):
        if table_row_count_update > i_row:
            result = api_xmlriver.process2(f'data\\{filename}')
...
def main():
    global table
    table = service_table.Table('1hkn1c0Y6IMkUg2m7orn0DTG-OojNnn9Najyki1RdY-s', sheetId=0)
